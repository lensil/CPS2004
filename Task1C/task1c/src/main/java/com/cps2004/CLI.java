...
public class CLI {
    ...
    public void restore() {
        // restore stock
        stock.load(new File("stock.json"));

        // restore packages
        ObjectMapper mapper = new ObjectMapper();
        JsonNode rootNode = null;
        try {
            rootNode = mapper.readTree(new File("packages.json"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        if (rootNode != null) {
        ...
        rootNode = null;
        try {
            rootNode = mapper.readTree(new File("customers.json"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        if (rootNode != null) {
            for (JsonNode node : rootNode) {
                ...
                customer.load((ObjectNode) node);
                customers.add(customer);
            }
        }

        // Restore Supplier state
        mapper = new ObjectMapper();
        rootNode = null;
        try {
            rootNode = mapper.readTree(new File("suppliers.json"));
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        rootNode = null;
        try {
            rootNode = mapper.readTree(new File("transports.json"));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    public void save() {
        // save stock
        stock.save(new File("stock.json"));
        ...
        for (Package package_ : packages) {
            ...
            package_.save(objectNode);
            arrayNode.add(objectNode);
        }
        try {
            mapper.writeValue(new File("packages.json"), arrayNode);
        } catch (IOException e) {
            e.printStackTrace();
        }

        // Save customers state
        mapper = new ObjectMapper();
        arrayNode = mapper.createArrayNode();
        for (Customer customer : customers) {
            ObjectNode objectNode = mapper.createObjectNode();
            customer.save(objectNode);
            arrayNode.add(objectNode);
        }
        try {
            mapper.writeValue(new File("customers.json"), arrayNode);
        } catch (IOException e) {
            e.printStackTrace();
        }

        // Save suppliers state
        mapper = new ObjectMapper();
        arrayNode = mapper.createArrayNode();
        for (Supplier supplier : suppliers) {
            ObjectNode objectNode = mapper.createObjectNode();
            supplier.save(objectNode);
            arrayNode.add(objectNode);
        }
        try {
            mapper.writeValue(new File("suppliers.json"), arrayNode);
        } catch (IOException e) {
            e.printStackTrace();
        }
        ...
        mapper = new ObjectMapper();
        arrayNode = mapper.createArrayNode();
        for (Transport transport_ : transport) {
            ObjectNode objectNode = mapper.createObjectNode();
            transport_.save(objectNode);
            arrayNode.add(objectNode);
        }
        try {
            mapper.writeValue(new File("transports.json"), arrayNode);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
