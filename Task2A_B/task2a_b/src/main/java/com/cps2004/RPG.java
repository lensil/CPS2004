...
public class RPG extends VideoGame{
    ...
    public void save(ObjectNode newItem){
        newItem.put("type", "rpg");
        super.save(newItem);
        newItem.put("story", this.story);
        newItem.put("multiplayer", this.multiplayer);
    }

    // Load product information from file
    public void load(JsonNode node){
        super.load(node);
        story = node.get("story").asText();
        multiplayer = node.get("multiplayer").asBoolean();
    }
