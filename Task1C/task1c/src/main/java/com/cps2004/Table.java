...
public class Table extends Furniture {
    ...
    public void save(ObjectNode newItem){
        newItem.put("type", "table");
        super.save(newItem);
        newItem.put("extendable", this.extendable);
        newItem.put("shape", this.shape);
        newItem.put("noOfLegs", this.noOfLegs);
    }

    // Load product information from file
    public void load(JsonNode node){
        super.load(node);
        extendable = node.get("extendable").asBoolean();
        shape = node.get("shape").asText();
        noOfLegs = node.get("noOfLegs").asInt();
    }
}
