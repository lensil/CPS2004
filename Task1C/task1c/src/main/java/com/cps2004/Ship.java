...
public class Ship extends Transport{
    ...
    public void save(ObjectNode newItem) {
        newItem.put("transportType", "ship");
        super.save(newItem);
        StringBuilder harboursString = new StringBuilder();
        for (int i = 0; i < harbours.size(); i++) {
            if (i == harbours.size() - 1) {
                harboursString.append(harbours.get(i));
            } else {
                harboursString.append(harbours.get(i)).append(", ");
            }
        }
        newItem.put("harbours", harboursString.toString());
    }

    // Load transport information from file
    public void load(JsonNode node) {
        super.load(node);
        String harboursString = node.get("harbours").asText();
        Collections.addAll(harbours, harboursString.split(", "));
    }
}